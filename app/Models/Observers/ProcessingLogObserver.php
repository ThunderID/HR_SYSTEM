...
class ProcessingLogObserver 
{
	public function saved($model)
	{
		...
		$date_on 						= Carbon::createFromFormat('Y-m-d', date('Y-m-d', strtotime($model['attributes']['on'])));
		...
		// 	//3. COUNT IDLE
		// 	$idle 					= Log::where('on', '>', date('Y-m-d H:i:s',strtotime($on)))->where('on', '<', date('Y-m-d H:i:s',strtotime($on.' + 1 day')))->personid($model['attributes']['person_id'])->orderBy('on', 'asc')->get();
