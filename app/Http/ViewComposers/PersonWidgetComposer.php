...
class PersonWidgetComposer extends WidgetComposer 
{
	...
	{
		$results 								=  $this->dispatch(new Getting(new PersonWidget, $options['search'], $options['sort'] , (int)$options['page'], (int)$options['per_page'], isset($options['new']) ? $options['new'] : false));
		...
		if(!$contents->meta->success)
		{
			foreach ($contents->meta->errors as $key => $value) 
			{
				if(is_array($value))
				{
					...
					{
						$this->widget_errors->add('PersonWidget', $value2);
					}
				}
				else
				{
					$this->widget_errors->add('PersonWidget', $value);
				}
			}

			$widget_data['widget'] 			= null;
			$widget_data['widget-pagination'] = null;
		}
		else
		{
			$page 								= json_decode(json_encode($contents->pagination), true);
			$widget_data['widget'] 			= json_decode(json_encode($contents->data), true);
			$widget_data['widget-pagination'] = new Paginator($page['total_data'], $page['total_data'], $page['per_page'], $page['page']);
			$widget_data['widget-display'] 	= $page;
		}
