...
class AttendanceLogComposer extends WidgetComposer 
{
	...
	protected function setData($options)
	{
		...
		$results 								=  $this->dispatch(new Getting(new AttendanceLog, $options['search'], $options['sort'] , (int)$options['page'], (int)$options['per_page'], isset($options['new']) ? $options['new'] : false));
		...
		if(!$contents->meta->success)
		{
			foreach ($contents->meta->errors as $key => $value) 
			{
				if(is_array($value))
				{
					...
					{
						$this->widget_errors->add('AttendanceLog', $value2);
					}
				}
				else
				{
					$this->widget_errors->add('AttendanceLog', $value);
				}
			}
			$widget_data['attendancelog'] 				= null;
			$widget_data['attendancelog-pagination'] 	= null;
		}
		else
		{
			$page 										= json_decode(json_encode($contents->pagination), true);
			$widget_data['attendancelog'] 				= json_decode(json_encode($contents->data), true);
			$widget_data['attendancelog-pagination'] 	= new Paginator($page['total_data'], $page['total_data'], $page['per_page'], $page['page']);
			$widget_data['attendancelog-display'] 		= $page;
		}
