...
class BranchComposer extends WidgetComposer 
{
	...
	protected function setData()
	{
		...
		$results 								=  $this->dispatch(new Getting(new Branch, $this->widget_data['search'], $this->widget_data['sort'] , $this->widget_data['page'], $this->widget_data['per_page']));
		...
		if(!$contents->meta->success)
		{
			...
			$this->widget_data['branch-'.$this->widget_data['identifier']] 				= null;
			$this->widget_data['branch-pagination-'.$this->widget_data['identifier']] 	= null;
		}
		else
		{
			$page 																		= json_decode(json_encode($contents->pagination), true);
			$this->widget_data['branch-'.$this->widget_data['identifier']] 				= json_decode(json_encode($contents->data), true);
			$this->widget_data['branch-pagination-'.$this->widget_data['identifier']] 	= new Paginator($page['total_data'], $page['total_data'], $page['per_page'], $page['page']);
			$this->widget_data['branch-pagination-'.$this->widget_data['identifier']]->setPath(route('hr.branches.index'));
			$this->widget_data['branch-display-'.$this->widget_data['identifier']] 		= $page;
		}
