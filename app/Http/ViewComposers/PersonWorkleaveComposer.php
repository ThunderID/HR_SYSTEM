...
class PersonWorkleaveComposer extends WidgetComposer 
{
	...
	{
		$results 								=  $this->dispatch(new Getting(new PersonWorkleave, $options['search'], $options['sort'] , (int)$options['page'], (int)$options['per_page'], isset($options['new']) ? $options['new'] : false));
		...
		if(!$contents->meta->success)
		{
			foreach ($contents->meta->errors as $key => $value) 
			{
				if(is_array($value))
				{
					...
					{
						$this->widget_errors->add('PersonWorkleave', $value2);
					}
				}
				else
				{
					$this->widget_errors->add('PersonWorkleave', $value);
				}
			}
