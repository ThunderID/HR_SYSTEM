...
class IdleLogComposer extends WidgetComposer 
{
	...
	protected function setData($options)
	{
		...
		$results 								=  $this->dispatch(new Getting(new IdleLog, $options['search'], $options['sort'] , (int)$options['page'], (int)$options['per_page'], isset($options['new']) ? $options['new'] : false));
		...
		if(!$contents->meta->success)
		{
			foreach ($contents->meta->errors as $key => $value) 
			{
				if(is_array($value))
				{
					...
					{
						$this->widget_errors->add('IdleLog', $value2);
					}
				}
				else
				{
					$this->widget_errors->add('IdleLog', $value);
				}
			}
		...
		else
		{
			...
			$widget_data['idlelog'] 			= json_decode(json_encode($contents->data), true);
			$widget_data['idlelog-pagination'] 	= new Paginator($page['total_data'], $page['total_data'], $page['per_page'], $page['page']);
			$widget_data['idlelog-display'] 	= $page;
		}
