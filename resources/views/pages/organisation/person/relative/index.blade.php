...
@section('content_filter')
	@include('widgets.common.filter', [
		'widget_template'		=> 'plain_no_title',
		'widget_options'		=> [
									'form_url'	=> route('hr.person.relatives.index', ['org_id' => $data['id'], 'person_id' => $person['id'], 'page' => (Input::has('page') ? Input::get('page') : 1)])
									],
	])
@overwrite

@section('content_body')	
	@include('widgets.organisation.person.relative.table', [
		'widget_template'		=> 'panel',
		'widget_title'			=> 'Data Kerabat "'.$person['name'].'"'.((Input::has('page') && (int)Input::get('page') > 1) ? '<small class="font-16"> Halaman '.Input::get('page').'</small>' : null),
		'widget_title_class'	=> 'text-uppercase ml-10 mt-20',
		'widget_body_class'		=> '',
		'widget_options'		=> 	[
										'relativelist'			=>
										[
											'search'			=> array_merge(['personid' => $person['id'], 'defaultcontact' => true, 'relativeorganisationid' => $data['id']], (isset($filtered['search']) ? $filtered['search'] : [])),
											'sort'				=> (isset($filtered['sort']) ? $filtered['sort'] : ['relationship' => 'desc']),
