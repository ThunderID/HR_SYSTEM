		...
		@if(isset($BranchComposer['widget_data']['branchlist']['branch']))
			...
			<table class="table table-hover table-affix">
				...
				<tbody>
					...
					@forelse($BranchComposer['widget_data']['branchlist']['branch'] as $key => $value)
						<tr>
							<td class="">
								{{$i}}
							</td>
							...
							<td class="">
								@foreach($value['contacts'] as $key2 => $value2)
									@if((strtolower($value2['item'])=='mobile')||(strtolower($value2['item'])=='mobile'))
										{{$value2['value']}}
									@endif
								@endforeach
							</td>
							...
							<td class=" text-center">
								<div class="btn-group">
									...
									<ul class="dropdown-menu dropdown-menu-right">
										...
										@if ((int)Session::get('user.menuid')<=2)
											<li>
												<a href="{{ route('hr.branch.fingers.index', [$value['id'], 'org_id' => $data['id'], 'branch_id' => $value['id']]) }}" title="pengaturan fingerprint"><i class="fa fa-hand-pointer-o"></i> Pengaturan Sidik Jari</a>
											</li>
										@endif
										<li>
